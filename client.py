...
class client():
    ...
    def list_publications(self, trusted_only=False, list_formatting=True):
        ...
        if not list_formatting:
            ...
            print("{:<29}|{:<10}|{:<12}|{:<10}|{:<19}|".format('ID','Version','Title','Creator','Updated time'))
        ...
        if not quiet:
            if len(res.devices)>0:
                ...
                for d in res.devices:
                    address = self.peerInfo(d, quiet=True)
                    devices[d]=address
            else:
...
def main():
    ...
    group_subparser = group_parser.add_subparsers(title="Manage Groups", required=True,
    ...
    document_subparser = document_parser.add_subparsers(title="Manage Documents", required=True,
